...
interface Props {
    ...
    onChangeSetExpires: (e:React.ChangeEvent<HTMLInputElement>) => void;
    onKeyDownSetExpires: (e:React.KeyboardEvent<HTMLInputElement>) => void;
    editText: string;
    editTextRef: React.RefObject<HTMLInputElement> | null;
    expires2: string;
    setExpires2: (value: string) => void;
    getDay: (expires: Date | null) => { label: string; color: string };
    ...
    onClickDeleteBtn2: (e: React.MouseEvent<HTMLButtonElement>, item: 할일목록Type)=> void;
}
                    ...
                    <li key={item.아이디} data-key={item.아이디}>
                            ...
                            (editId !==null && editId === item.아이디) ?
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={onChangeSetExpires}
                                        onKeyDown={onKeyDownSetExpires}
                                    />
                                </div>
                                :
                                <em className='left'>
                                    ...
                                    <strong>
                                        <i style={{  color: getDay(item.만료일).color }}>{getDay(item.만료일).label}</i>
                                    </strong>
                        ...
                        {   
                            ...
                            <button className='edit' onClick={(e) => onClickEdit(e, item)}>
                            {
                                ...
                                :
                                !showDeleted && <i className="bi bi-pencil"></i>
                            }
                            </button>
                            ...
                            
                        {
                            showDeleted ? 
                                (
                                    ...
                                    &nbsp;&nbsp;&nbsp;
                                    <button 
                                        onClick={(e)=>onClickDeleteBtn2(e, item)}
                                        className='delete-btn'
                                    ><i className="bi bi-x-lg" style={{color:'#c00'}}></i></button>
                                    </>
                                )
                                :                                    
                                (
